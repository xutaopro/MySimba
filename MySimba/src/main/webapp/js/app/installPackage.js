var InstallPackage = {
	...
	"toAdd" : function() {
		window.self.location.href = contextPath + "/install/toAdd.do";
	},

	"toUpdate" : function(installVersion) {
		window.self.location.href = contextPath + "/install/toUpdate.do?installVersion=" + installVersion;
	},
	...
	"batchDelete" : function() {
		...
		$.ajax({
			url : contextPath + "/install/batchDelete.do?json",
			type : "post",
			dataType : "json",
			async : true,
			data : {
	...
	"add" : function() {
		...
		$("#addForm").attr("action", contextPath + "/install/add.do");
		$("#addForm").submit();
	},

	"update" : function() {
		$("#updateForm").form('submit', {
			url : contextPath + "/install/update.do?json",
			onSubmit : function() {
	...
	"updateNewest" : function(installVersion) {
		$.ajax({
			url : contextPath + "/install/updateNewest.do?json",
			type : "post",
			dataType : "json",
			async : true,
			data : {
	...
	"toList" : function() {
		window.self.location.href = contextPath + "/install/list.do";
	},
